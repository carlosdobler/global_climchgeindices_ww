...
# MODEL LOOP
for(mod in c("Had", "MPI", "Nor")[3]){                                                      ### REMOVE
  ...
  dir_down <- str_glue("~/pers_disk/{dom}")
  dir.create(dir_down)
  ...
  pwalk(chunks_ind, function(lon_ch, lat_ch, r, ...){
    ...
    walk(c("tasmax", "tasmin", "pr"), function(v){
      ...
      print(str_glue("[{dom}] [{mod}] Importing {v}"))
      ...
      # tic(str_glue("   Sliced {length(files_vector)} files"))
      iwalk(files_vector, function(f, i){
        
        tic(str_glue("   Sliced file {i} / {length(files_vector)}"))
        
        str_glue("{dir_tmp_vars}/{str_pad(i, 2, 'left', '0')}_{v}_cut.nc") -> outfile
        
        system(
          str_glue(
            "cdo selindexbox,{lon_chunks[[lon_ch]][1]},{lon_chunks[[lon_ch]][2]},{lat_chunks[[lat_ch]][1]},{lat_chunks[[lat_ch]][2]} {f} {outfile}"
          ),
          ignore.stderr = T
        )
      ...
      str_glue("{dir_tmp_vars}/{v}_stacked.nc") -> outfile
    ...
    list.files(dir_tmp_var, full.names = T) %>%
      {c(.[str_detect(., "tasmax")], .[str_detect(., "tasmin")], .[str_detect(., "pr")])} %>% 
      map(readRDS) -> s
    
    unlink(dir_tmp_var, recursive = T)
    ...
    walk2(seq(1, yrs*27, yrs), 
          ...
          function(t, i){
            ...
            s_etccdi %>% 
              ...
              saveRDS(str_glue("{dir_tmp}/star_{i}_{lon_ch}_{lat_ch}.rds"))
  ...
  c("fd", "su", "id", "tr",
    ...
    walk(function(v){
      ...
      list.files(dir_tmp, full.names = T) %>% 
        .[str_detect(., v)] %>% 
        map(readRDS) -> chunks
